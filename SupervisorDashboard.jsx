...
export default function SupervisorDashboard() {
  ...
  return (
    <div className="p-6 bg-gray-900 text-white min-h-screen">
      <h1 className="text-2xl mb-4">لوحة المشرف</h1>
      <input
        type="text"
      ...
      {result && result.map((m) => (
        <div key={m.id} className="mt-4 p-4 border border-white rounded">
          ...
          <button className="bg-purple-600 px-2 py-1 rounded">تنزيل شهادة</button>
          <button className="bg-purple-600 px-2 py-1 rounded">تنزيل بطاقة</button>
        </div>
      ))}
    </div>
  );
