...
function showMemberDetail(m) {
  ...
  area.innerHTML = `
    <div class="card">
      ...
      <div class="grid-actions">
        ...
        <button class="btn" id="renewBtn">تجديد العضوية</button>
        <button class="btn danger" id="banBtn">${m.status === "banned" ? "إلغاء الحظر" : "حظر"}</button>
      </div>
    </div>
  `;
  ...
  document.getElementById("renewBtn").onclick = () => doRenew(m);
  document.getElementById("banBtn").onclick = () => doBanToggle(m);
}
...
function showRankChangeForm(m) {
  ...
  document.getElementById("rankSave").onclick = async () => {
    ...
    const { data, error } = await sb.rpc("fn_change_rank", { p_token: session.token, p_member_code: m.membership_code, p_new_rank_code: newRank });
